...
public class check{
    ...
    public check(){} public static String Balance(String s)
    {
        ...
        int last_coeff=Math.abs(sum/(int)B[count][0]);
        
        int z = r.length;
        int final_coeff[] = new int[z];
        for(int i=0;i<prod.length;i++)
        {
            final_coeff[i]=(int)Math.round(Math.abs(prod[i][0]));
        }
        final_coeff[z-1]=(int)Math.round(Math.abs(last_coeff));

        int f= factor(final_coeff);
